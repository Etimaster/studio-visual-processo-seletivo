...
foreach ($arr_unique as $word) {
	$count_world++;
	$arr_word = str_split($word);
	$val_decimal = 0;
	foreach ($arr_word as $key => $casa_vigesimal) {
		$val_casa_vigesimal = array_search($casa_vigesimal, $arr_alfabeto);
		$val_casa_decimal = pow(20, $key+1) * $val_casa_vigesimal / 20;

		$val_decimal += $val_casa_decimal;
	}

	$is_big = $val_decimal >= 422224;
	$divisible_by_3 = $val_decimal % 3 == 0;
	if($is_big && $divisible_by_3){
		$count++;
	}
}
