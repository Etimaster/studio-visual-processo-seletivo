...
foreach ($arr as $value){
    $have_5_char = strlen($value) >= 8;
    $last = substr($value, -1);
    $last_is_foo = in_array($last, $foo_letters);
	$first = substr($value, 0, 1);
    $first_is_foo = in_array($first, $foo_letters);
    $have_l = stripos($value, 'l') !== false;

    if($have_5_char && !$last_is_foo){
        $count++;
